...
public class IconPackHelper {
            ...
            builder.setAdapter(adapter, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int position) {
                    ...
                    PreferenceManager.getDefaultSharedPreferences(context).edit()
                            .putString(CyraPreferencesProvider.KEY_ICON_PACK, selectedPackage).commit();
    ...
    public int getResourceIdForActivityIcon(String pkgname, String name) {
        String drawable = mIconPackResources.get(pkgname.toLowerCase()
                + "." + name.toLowerCase());
        if (drawable == null) {
            // Icon pack doesn't have an icon for the activity, fallback to package icon
            drawable = mIconPackResources.get(pkgname.toLowerCase());
            if (drawable == null) {
                return 0;
            }
        }
